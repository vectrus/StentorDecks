...
- R2.2 Jog wheels: nudge (temporary tempo bend while playing) and seek (while paused) only. No scratching. Latency target ≤ 40 ms output; no ASIO requirement. **Mouse path (R1.5):** drag/click the deck detail or overview waveform to scrub the playhead; double-click while paused sets the cue at that position (no jog required).
...
- R4.1 Load into a deck via: hardware load buttons, per-deck UI Load button, double-click in browser, or **drag a library track onto that deck’s waveform** (Performance) / deck strip (Library) — mouse path when no controller (R1.5).
