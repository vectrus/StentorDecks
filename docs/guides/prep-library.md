# Library mode — how to use it (simple guide)

This page is for **you**, the DJ — not for programmers.  
It explains what the **Library** screen is, and how to use it.
...
| **Library** | Getting your music ready: find tracks, fix BPM & key, load decks. |
...
3. Shows them in Library so you can browse and load  
...
## The Library screen — what each part does
...
- A dimmed row with a **`✓`** means you already **played** that track this session (Play on for at least half a minute). Cleared when you quit the app, or with **Clear session played** (Library footer or Settings → Library).
...
Library itself focuses on browsing + fixing; Load B from the mouse is easiest in Performance (or use the Hercules Load buttons).
...
So: dig in Library with the mouse, then keep browsing from the controller in Performance without losing your place.
...
## Library vs Performance (same brain)
...
| Folder tree | Yes | No (search + list; open folders from Library or MIDI) |
...
*(Later idea — parked for v2: maybe reuse this Library list as a “next songs” queue when AI Co-pilot helps pick tracks. See [`../BACKLOG-v2-spotify-ai.md`](../BACKLOG-v2-spotify-ai.md) § Prep library as queue.)*

**Does Library mode delete files?**  
No. Never.
...
- Library layout: [`../06-ui-style-guide.md`](../06-ui-style-guide.md), mockup [`../mockups/02-prep-mode.html`](../mockups/02-prep-mode.html)
