...
4. Booth safety details: confirm-on-quit while any deck is playing; screen-sleep/display-off inhibited for the whole app session via `powerSaveBlocker('prevent-display-sleep')` (Windows idle dim/blank — prep and performance); audio continues when window is minimized/unfocused (backgroundThrottling off, verified).
