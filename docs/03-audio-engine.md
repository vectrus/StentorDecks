...
- Position for UI/waveform derived each rAF: re-anchor on rate change; while ramping, multiply elapsed by the **live** `playbackRate.value` so the playhead does not race the audio.
- Nudge (jog) — **dual-zone** (`shared/jogFeel`, tunables in `settings.mixer.jog` / docs/07): **fine** fingertip vs **spin** (high tick-rate EMA or large packed `|delta|`). RMX2 floods relative ±1 CCs — fine uses flood compression + a short **impulse cap** (~0.4 ms sticky phase per ~50 ms window) so a light nudge feels like a heavy platter; playing seeks are **coalesced to one `seek` per rAF** with a short input soft-start (fewer clicks). Soft defaults ~0.08→12 ms seek, 0.02→4 % rate, spin opens ~140→320 t/s. `dualZone: false` forces fine-only. Settings: Soft (heavy platter) / Balanced / Spinny. Jog mutes soft phase assist ~300 ms and retargets post-SYNC phase glue.
