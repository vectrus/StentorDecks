...
- Nudge (jog) — **dual-zone** (`shared/jogFeel`, tunables in `settings.mixer.jog` / docs/07): **fine** vs **spin**. RMX2 jog ≈ 9 cm (vs ~30 cm SL-1200): fine **steady light turns are rate-primary** (smooth crawl — sticky seeks suppressed so it does not chunk like vinyl slip); short bursts get a tiny diameter-scaled impulse (~0.12 ms cap). Spin still throws on a hard whip. Playing seeks coalesce per rAF when used. Soft defaults ~0.025→12 ms seek, 0.035→4 % rate, spin ~150→340 t/s. Settings: Soft (small heavy wheel) / Balanced / Spinny. Jog mutes soft phase assist ~300 ms and retargets post-SYNC phase glue.
