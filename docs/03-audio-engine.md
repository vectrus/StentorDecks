...
- SYNC: latching on/off, **one slave at a time** — engage clears SYNC on the partner, then matches tempo so pitchOnlyBPM(this) = pitchOnlyBPM(other) (fileBPM × pitch-fader rate; partner jog/bend ignored) or pitch-% match if file BPM unknown, then **one-shot phase snap** (seek this deck by shortest delta so beat phase at the playhead matches the other; grid origin 0:00, period = 60/pitchOnlyBPM). Stays armed for tempo follow only (no continuous phase-lock). Released by pressing SYNC again (leave pitch frozen for manual jogs), loading (R3.3), or moving the slave's pitch fader. Soft-takeover re-arms the pitch fader on engage. Factory Sync buttons only — never aliased to tap tempo. Phase snap skipped when BPM is unknown (pitch-% mode).
