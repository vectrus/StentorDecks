...
- SYNC: latching on/off — engage matches tempo so effectiveBPM(this) = effectiveBPM(other) (or pitch-% match if file BPM unknown) and stays armed (tempo follow while on; LED lit). Released by pressing SYNC again, loading (R3.3), or moving the pitch fader. Soft-takeover re-arms the pitch fader on engage. Factory Sync buttons only — never aliased to tap tempo.
