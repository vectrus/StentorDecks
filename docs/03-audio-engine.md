...
- SYNC: latching on/off — engage matches tempo so effectiveBPM(this) = effectiveBPM(other) (or pitch-% match if file BPM unknown), then **one-shot phase snap** (seek this deck by shortest delta so beat phase at the playhead matches the other; grid origin 0:00, period = 60/effectiveBPM). Stays armed for tempo follow only (no continuous phase-lock). Released by pressing SYNC again, loading (R3.3), or moving the pitch fader. Soft-takeover re-arms the pitch fader on engage. Factory Sync buttons only — never aliased to tap tempo. Phase snap skipped when BPM is unknown (pitch-% mode).
