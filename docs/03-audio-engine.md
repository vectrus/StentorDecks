...
- Nudge (jog) — `shared/jogFeel`, tunables in `settings.mixer.jog` / docs/07. **Vinyl OFF / `dualZone: false` (default Soft):** playing = **tempo nudge only** (R2.2) — constant temporary rate bend per MIDI message (no tick-rate “gear”); no sticky seeks while playing. **Vinyl ON / `dualZone: true`:** dual-zone sticky phase — **fine** = quiet seek-primary (fine rate forced 0); flood compression + impulse cap (~0.22 ms / 50 ms); **spin** opens only on sustained message rate (packed |delta| alone does not). Scratch CCs `32`/`33` alias to turn jogs. Playing seeks coalesce per rAF into one micro crossfade seek. RMX2 **Vinyl** note `0x47` toggles dual-zone (LED follows). Settings presets: Soft (CDJ nudge) / Balanced (Vinyl dual) / Spinny. Jog mutes soft phase assist ~800 ms and retargets post-SYNC phase glue.
