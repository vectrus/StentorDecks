...
export class DeckStore {
  ...
  nudge(velocity: number): void {
    ...
    if (this.state === 'playing') {
      // Soft: slow rim = ride (rate); faster rim = sticky chunk. Vinyl: fine/spin.
      // Seeks coalesce per rAF (flushJogSeek) — never one seek per MIDI message.
      if (scaled.playingSeekSec > 0) {
